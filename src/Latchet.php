...
use Sidney\Latchet\Exceptions\ChannelNotFoundException;
use Sidney\Latchet\Exceptions\LatchetException;
...
class Latchet implements WampServerInterface
{
    ...
    public function __construct(Container $container = null)
    {
        ...
        $this->enablePush = config('latchet.enablePush');
    ...
    protected function connectZmq()
    {
        ...
        $this->socket = $context->getSocket(\ZMQ::SOCKET_PUSH,
            config('latchet.zmqPushId', sprintf('latchet.push.%s', \App::environment())));
        $this->socket->connect("tcp://localhost:" . config('latchet.zmqPort'));
    ...
    /**
     * Redirect serverPublish to LatchetPusher
     *
     ...
     * @param array $variables
     *
     * @return \Sidney\Latchet\TopicEventHandler
     */
